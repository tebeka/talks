...
def link2dict(link):
    columns = [col.name for col in Link.__table__.columns]
    return dict((col, getattr(link, col)) for col in columns)


@app.route('/api/v1/top/<count>')
def top(count):
    try:
        count = int(count)
    except ValueError:
    ...
    links = db.session.query(Link).order_by(-Link.hits).limit(count)
    return jsonify(rows=[link2dict(link) for link in links])
